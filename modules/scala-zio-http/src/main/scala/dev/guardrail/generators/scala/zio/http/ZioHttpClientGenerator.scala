...
class ZioHttpClientGenerator(version: ZioHttpVersion) extends ClientTerms[ScalaLanguage, Target] {
      ...
      for {
        ...
        embedMultipart =
          formDataParams
            ...
            .map { formDataParams => (inner: List[Stat]) =>
  ...
  def clientClsArgs(tracingName: Option[String], serverUrls: Option[NonEmptyList[URI]], tracing: Boolean): Target[List[List[scala.meta.Term.Param]]] = {
    val ihc = param"implicit httpClient: ZioHttpClient[F]"
  ...
  def buildStaticDefns(
    ...
    def extraConstructors(
      ...
      List(
        q"""
            def httpClient[F[_]](httpClient: ZioHttpClient[F], ${formatHost(serverUrls)}, ..${tracingParams})(implicit F: Async[F]): ${tpe}[F] = ${ctorCall}
          """
      )
    }
