...
class ZioHttpServerGeneratorLoader extends ServerGeneratorLoader {
  ...
  val apply            = ModuleLoadResult.forProduct1(ServerGeneratorLoader.label -> Seq(ZioHttpVersion.mapping))(version => ZioHttpServerGenerator(version))
}
...
class ZioHttpServerGenerator private (version: ZioHttpVersion) extends ServerTerms[ScalaLanguage, Target] {
  ...
  def pathStrToZioHttp(basePath: Option[String], path: Tracker[String], pathArgs: List[LanguageParameter[ScalaLanguage]]): Target[(Pat, Option[Pat])] =
    (basePath.getOrElse("") + path.unwrapTracker).stripPrefix("/") match {
      case ""        => Target.pure((p"${Term.Name("Root")}", None))
      case finalPath => ZioHttpServerGenerator.generateUrlPathExtractors(Tracker.cloneHistory(path, finalPath), pathArgs, CirceModelGenerator.V012)
    }
  ...
  def bodyToZioHttp(methodName: String, body: Option[LanguageParameter[ScalaLanguage]]): Target[Option[Term => Term]] =
    Target.pure(
  ...
  def headersToZioHttp: List[LanguageParameter[ScalaLanguage]] => Target[List[Param]] =
  ...
  def qsToZioHttp(methodName: String): List[LanguageParameter[ScalaLanguage]] => Target[Option[Pat]] =
    params =>
      directivesFromParams(
  ...
  def formToZioHttp: List[LanguageParameter[ScalaLanguage]] => Target[List[Param]] =
    directivesFromParams(
      arg => {
        case t"String" =>
          Target.pure(
  ...
  def asyncFormToZioHttp(methodName: String): List[LanguageParameter[ScalaLanguage]] => Target[List[Param]] =
    directivesFromParams(
      arg =>
        elemType =>
          if (arg.isFile) {
  ...
  private def generateRoute(
    ...
    case GenerateRouteMeta(
      ...
      Target.log.function("generateRoute")(for {
        ...
        http4sMethod <- httpMethodToZioHttp(method)
        pathWithQs   <- pathStrToZioHttp(basePath, path, pathArgs)
        (http4sPath, additionalQs) = pathWithQs
        http4sQs   <- qsToZioHttp(methodName)(qsArgs)
        http4sBody <- bodyToZioHttp(methodName, bodyArgs)
        asyncFormProcessing = formArgs.exists(_.isFile)
        http4sForm         <- if (asyncFormProcessing) asyncFormToZioHttp(methodName)(formArgs) else formToZioHttp(formArgs)
        http4sHeaders      <- headersToZioHttp(headerArgs)
  ...
  def createZioHttpHeaders(headers: List[Header[ScalaLanguage]]): (Term.Name, List[Defn.Val]) = {
